...
using Smartobjecttokenlauncher;
using Smartobjecttokenlauncher.Program;
using Smartobjecttokenlauncher.Errors;
using Smartobjecttokenlauncher.Accounts;
using Smartobjecttokenlauncher.Types;

namespace Smartobjecttokenlauncher
{
    namespace Accounts
    {
        ...
        public partial class SmartObjectTokenLauncher
        {
            public static ulong ACCOUNT_DISCRIMINATOR => 15127388163727664250UL;
            public static ReadOnlySpan<byte> ACCOUNT_DISCRIMINATOR_BYTES => new byte[]{122, 164, 111, 246, 98, 71, 239, 209};
            public static string ACCOUNT_DISCRIMINATOR_B58 => "MWnVFx7jDZJ";
            ...
            public static SmartObjectTokenLauncher Deserialize(ReadOnlySpan<byte> _data)
            {
                ...
                SmartObjectTokenLauncher result = new SmartObjectTokenLauncher();
    ...
    namespace Errors
    {
        public enum SmartobjecttokenlauncherErrorKind : uint
        {
        }
    }
    ...
    public partial class SmartobjecttokenlauncherClient : TransactionalBaseClient<SmartobjecttokenlauncherErrorKind>
    {
        public SmartobjecttokenlauncherClient(IRpcClient rpcClient, IStreamingRpcClient streamingRpcClient, PublicKey programId = null) : base(rpcClient, streamingRpcClient, programId ?? new PublicKey(SmartobjecttokenlauncherProgram.ID))
        {
        }

        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<Entity>>> GetEntitysAsync(string programAddress = SmartobjecttokenlauncherProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectTokenLauncher>>> GetSmartObjectTokenLaunchersAsync(string programAddress = SmartobjecttokenlauncherProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
            var list = new List<Solana.Unity.Rpc.Models.MemCmp>{new Solana.Unity.Rpc.Models.MemCmp{Bytes = SmartObjectTokenLauncher.ACCOUNT_DISCRIMINATOR_B58, Offset = 0}};
            var res = await RpcClient.GetProgramAccountsAsync(programAddress, commitment, memCmpList: list);
            if (!res.WasSuccessful || !(res.Result?.Count > 0))
                return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectTokenLauncher>>(res);
            List<SmartObjectTokenLauncher> resultingAccounts = new List<SmartObjectTokenLauncher>(res.Result.Count);
            resultingAccounts.AddRange(res.Result.Select(result => SmartObjectTokenLauncher.Deserialize(Convert.FromBase64String(result.Account.Data[0]))));
            return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectTokenLauncher>>(res, resultingAccounts);
        }
        ...
        public async Task<Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectTokenLauncher>> GetSmartObjectTokenLauncherAsync(string accountAddress, Commitment commitment = Commitment.Finalized)
        {
            var res = await RpcClient.GetAccountInfoAsync(accountAddress, commitment);
            if (!res.WasSuccessful)
                return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectTokenLauncher>(res);
            var resultingAccount = SmartObjectTokenLauncher.Deserialize(Convert.FromBase64String(res.Result.Value.Data[0]));
            return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectTokenLauncher>(res, resultingAccount);
        }
        ...
        public async Task<SubscriptionState> SubscribeSmartObjectTokenLauncherAsync(string accountAddress, Action<SubscriptionState, Solana.Unity.Rpc.Messages.ResponseValue<Solana.Unity.Rpc.Models.AccountInfo>, SmartObjectTokenLauncher> callback, Commitment commitment = Commitment.Finalized)
        {
            SubscriptionState res = await StreamingRpcClient.SubscribeAccountInfoAsync(accountAddress, (s, e) =>
            {
                SmartObjectTokenLauncher parsingResult = null;
                if (e.Value?.Data?.Count > 0)
                    parsingResult = SmartObjectTokenLauncher.Deserialize(Convert.FromBase64String(e.Value.Data[0]));
                callback(s, e, parsingResult);
            }, commitment);
            return res;
        }

        protected override Dictionary<uint, ProgramError<SmartobjecttokenlauncherErrorKind>> BuildErrorsDictionary()
        {
            return new Dictionary<uint, ProgramError<SmartobjecttokenlauncherErrorKind>>{};
        }
    }
    ...
    namespace Program
    {
        ...
        public static class SmartobjecttokenlauncherProgram
        {
            public const string ID = "8va4yKEBACkT49C9wo94gS8ZaTdUrq2ipLgZvSNxWbd3";
