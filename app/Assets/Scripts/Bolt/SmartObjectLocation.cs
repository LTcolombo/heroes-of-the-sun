...
namespace SmartObjectLocation
{
    ...
    public partial class SmartObjectLocationClient : TransactionalBaseClient<SmartObjectLocationErrorKind>
    {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SessionToken>>> GetSessionTokensAsync(string programAddress = SmartObjectLocationProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
        ...
        public async Task<Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation.Accounts.SmartObjectLocation>>> GetSmartObjectLocationsAsync(string programAddress = SmartObjectLocationProgram.ID, Commitment commitment = Commitment.Confirmed)
        {
            var list = new List<Solana.Unity.Rpc.Models.MemCmp>{new Solana.Unity.Rpc.Models.MemCmp{Bytes = SmartObjectLocation.Accounts.SmartObjectLocation.ACCOUNT_DISCRIMINATOR_B58, Offset = 0}};
            var res = await RpcClient.GetProgramAccountsAsync(programAddress, commitment, memCmpList: list);
            if (!res.WasSuccessful || !(res.Result?.Count > 0))
                return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation.Accounts.SmartObjectLocation>>(res);
            List<SmartObjectLocation.Accounts.SmartObjectLocation> resultingAccounts = new List<SmartObjectLocation.Accounts.SmartObjectLocation>(res.Result.Count);
            resultingAccounts.AddRange(res.Result.Select(result => SmartObjectLocation.Accounts.SmartObjectLocation.Deserialize(Convert.FromBase64String(result.Account.Data[0]))));
            return new Solana.Unity.Programs.Models.ProgramAccountsResultWrapper<List<SmartObjectLocation.Accounts.SmartObjectLocation>>(res, resultingAccounts);
        }
        ...
        public async Task<Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation.Accounts.SmartObjectLocation>> GetSmartObjectLocationAsync(string accountAddress, Commitment commitment = Commitment.Finalized)
        {
            var res = await RpcClient.GetAccountInfoAsync(accountAddress, commitment);
            if (!res.WasSuccessful)
                return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation.Accounts.SmartObjectLocation>(res);
            var resultingAccount = SmartObjectLocation.Accounts.SmartObjectLocation.Deserialize(Convert.FromBase64String(res.Result.Value.Data[0]));
            return new Solana.Unity.Programs.Models.AccountResultWrapper<SmartObjectLocation.Accounts.SmartObjectLocation>(res, resultingAccount);
        }
        ...
        public async Task<SubscriptionState> SubscribeSmartObjectLocationAsync(string accountAddress, Action<SubscriptionState, Solana.Unity.Rpc.Messages.ResponseValue<Solana.Unity.Rpc.Models.AccountInfo>, SmartObjectLocation.Accounts.SmartObjectLocation> callback, Commitment commitment = Commitment.Finalized)
        {
            SubscriptionState res = await StreamingRpcClient.SubscribeAccountInfoAsync(accountAddress, (s, e) =>
            {
                SmartObjectLocation.Accounts.SmartObjectLocation parsingResult = null;
                if (e.Value?.Data?.Count > 0)
                    parsingResult = SmartObjectLocation.Accounts.SmartObjectLocation.Deserialize(Convert.FromBase64String(e.Value.Data[0]));
                callback(s, e, parsingResult);
            }, commitment);
            return res;
        }
